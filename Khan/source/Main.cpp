...
static VkPhysicalDevice         g_Gpu = nullptr;
...
{
    const VkSwapchainKHR old_swapchain = g_Swapchain;
    ...
    // Create Swapchain:
    {
        ...
        check_vk_result(err);
        err = vkGetSwapchainImagesKHR(g_Device, g_Swapchain, &g_BackBufferCount, nullptr);
        check_vk_result(err);
...
static void setup_vulkan(GLFWwindow* window)
{
    ...
    // Get Surface Format
    {
        ...
        uint32_t count;
        vkGetPhysicalDeviceSurfaceFormatsKHR(g_Gpu, g_Surface, &count, nullptr);
        VkSurfaceFormatKHR *formats = static_cast<VkSurfaceFormatKHR*>(malloc(sizeof(VkSurfaceFormatKHR) * count));
    ...
    // Get Present Mode
    {
        ...
        VkPresentModeKHR* presentModes = static_cast<VkPresentModeKHR*>(malloc(sizeof(VkQueueFamilyProperties) * count));
...
static void frame_end()
{
    vkCmdEndRenderPass(g_CommandBuffer[g_FrameIndex]);
    {
        ...
        VkResult err = vkEndCommandBuffer(g_CommandBuffer[g_FrameIndex]);
        check_vk_result(err);
...
#ifdef IMGUI_UNLIMITED_FRAME_RATE
    const uint32_t PresentIndex = (g_FrameIndex + IMGUI_VK_QUEUED_FRAMES - 1) % IMGUI_VK_QUEUED_FRAMES;
#else
...
int main(int, char**)
{
    ...
    // Upload Fonts
    {
        VkResult err = vkResetCommandPool(g_Device, g_CommandPool[g_FrameIndex], 0);
        check_vk_result(err);
